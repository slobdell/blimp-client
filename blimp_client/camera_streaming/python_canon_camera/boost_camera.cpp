...
void CameraManager::customize_settings(){
    return;
    const char* imagesize = "medium 3";
    CameraWidget *widget_imgsettings, *widget_main, *widget_image_size;
    gp_camera_get_config(this->camera, &widget_main, this->context);
    gp_widget_get_child_by_name(widget_main, "imgsettings", &widget_imgsettings);

    gp_widget_get_child_by_name(widget_imgsettings, "resolution", &widget_image_size);
    gp_widget_set_value(widget_image_size, imagesize);
    gp_camera_set_config(this->camera, widget_main, this->context);

}
bool CameraManager::initialize(){
    initialize_env();
    this->camera_ready = init_camera_and_context(this->camera, this->context);
    if(this->camera_ready == 1){
